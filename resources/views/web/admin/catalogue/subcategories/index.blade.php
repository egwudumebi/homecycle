...
@section('admin_title', 'Subcategories')
    ...
    <div class="mb-6">
        <div class="flex items-start justify-between gap-4">
            <div>
                ...
                <h2 class="mt-1 text-xl font-extrabold tracking-tight text-slate-900">Subcategories</h2>
                <p class="mt-2 text-sm text-slate-600">Browse subcategories and jump to listings by subcategory.</p>
            </div>
            <a href="{{ route('admin.catalogue.categories.index') }}" class="inline-flex h-10 items-center justify-center rounded-xl border border-slate-200 bg-white px-4 text-xs font-bold text-slate-800 hover:bg-slate-50">View categories</a>
        </div>
    </div>

    <div class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        <div class="flex items-center justify-between gap-3 border-b border-slate-100 bg-slate-50 px-5 py-4">
            <div class="text-xs font-bold uppercase tracking-widest text-slate-500">All subcategories</div>
            <div class="text-xs font-semibold text-slate-500">{{ $subCategories->total() }} total</div>
        </div>
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-slate-100">
                ...
                <tr>
                    ...
                    <th class="px-5 py-3 text-left text-[11px] font-extrabold uppercase tracking-widest text-slate-400">Category</th>
                ...
                @forelse($subCategories as $sc)
                    <tr class="hover:bg-slate-50">
                        ...
                        <td class="px-5 py-4">
                            <a href="{{ $sc->category ? route('admin.catalogue.categories.show', ['category' => $sc->category->id]) : '#' }}" class="font-bold text-slate-800 hover:text-indigo-600">
                                {{ $sc->category?->name ?? 'N/A' }}
                            </a>
                        </td>
                ...
                @empty
                    <tr>
                        <td colspan="6" class="px-5 py-10 text-center text-sm text-slate-500">No subcategories found.</td>
                    </tr>
                @endforelse
                </tbody>
            </table>
